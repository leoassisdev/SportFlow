...
router.get('/:id/history', (req, res, next) => matchController.listHistory(req, res).catch(next));
router.patch('/:id/score', (req, res, next) => matchController.updateScore(req, res).catch(next));
router.post('/:id/score/undo', (req, res, next) => matchController.undoLast(req, res).catch(next));
router.patch('/:id/timer', (req, res, next) => matchController.updateTimer(req, res).catch(next));
router.post('/:id/finish', (req, res, next) => matchController.finish(req, res).catch(next));
